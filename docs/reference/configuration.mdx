...
Use `rate_limit_requests` together with `rate_limit_period_seconds` to throttle outbound Gemini
requests (e.g., 30 calls per minute). This helps prevent `429 Too Many Requests` responses during
agent workflows that rapidly chain multiple tool calls.
