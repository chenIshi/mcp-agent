...
<Tabs>
  <Tab title="Gemini API">
    ...
    google:
      ...
      temperature: 0.7
      vertexai: false
      # Optional throttling to avoid 429 errors
      rate_limit_requests: 15           # Requests per window
      rate_limit_period_seconds: 60     # Window length in seconds
    ```
    ...
    <Note>
      ...
      Configure `rate_limit_requests`/`rate_limit_period_seconds` if you routinely see 429s.
    </Note>
  </Tab>
